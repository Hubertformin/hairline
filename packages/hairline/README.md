...
Hairline is a small, opinionated component library and token system. It ships three
...
![The Hairline specimen page — foundations, core, forms and data components](docs/specimen.png)
...
Hairline was extracted from the mockups for **Moni**, a personal-finance app for
...
<div class="hl-tile hl-tile--dark">
  <div class="hl-tile__head">
    <span class="hl-tile__label">Available</span>
  </div>
...
<button class="hl-btn hl-btn--solid">Record it</button>
```
...
```tsx
import { Tile, StatBlock, Button, DataRow } from '@hairline/ds';
import '@hairline/ds/hairline.css'; // once, at your app root
...
```tsx
import { Tile, StatBlock, Button, theme } from '@hairline/ds/native';
```
...
76 classes, all prefixed `hl-`, following BEM-ish `block__element--modifier`.

```
.hl-btn  .hl-btn--solid|quiet|ghost|danger  .hl-btn--s|l
.hl-iconbtn  .hl-chip  .hl-badge
.hl-field  .hl-select  .hl-toggle-row  .hl-tabs
.hl-tile  .hl-stat  .hl-row  .hl-meter
.hl-board  .hl-label  .hl-data  .hl-marker  .hl-hairline
```
...
| Page | What it shows |
| --- | --- |
| `docs/index.html` | Every component, in plain HTML styled only by `hairline.css`. |
...
scripts/
  ...
  build-css.ts     assembles dist/hairline.css and dist/tokens.css
...
No. `dist/hairline.css` is the foundation and works anywhere. React is a convenience
layer over it.
