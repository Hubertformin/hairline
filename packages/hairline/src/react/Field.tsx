...
export const Field = forwardRef<HTMLDivElement, FieldProps>(function Field(
  ...
  return (
    <div
      ref={ref}
      className={cx('hl-field', size === 'lg' && 'hl-field--lg', className)}
      {...rest}
    >
      {label ? <div className="hl-field__label">{label}</div> : null}
      <div className="hl-field__box">
        {prefix ? <span className="hl-field__prefix">{prefix}</span> : null}
        {editable ? (
          <input
            className="hl-field__value"
            value={typeof value === 'string' || typeof value === 'number' ? value : undefined}
            {...inputProps}
          />
        ) : (
          <span className="hl-field__value">{value}</span>
        )}
        {trailing}
      </div>
      {hint ? <div className="hl-field__hint">{hint}</div> : null}
    </div>
  );
});
