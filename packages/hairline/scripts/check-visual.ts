 ...
 * reimplementation — `.hl-tile--dark .hl-stat__value` has no utility form and has to
...
const INTENTIONAL: Record<string, { maxRatio: number; reason: string }> = {
  StatBlock: {
    maxRatio: 0.06,
    reason:
      'the export spreads its size style after setting font-variant-numeric, so the CSS `font` ' +
      'shorthand resets it and every figure except size "m" loses tabular alignment; ours keeps ' +
      'it, and tabular digits sit differently from proportional ones',
  },
  Tile: {
    maxRatio: 0.03,
    reason:
...
async function run(browser: Browser, url: string) {
  ...
  /** Every component that differed at all, whether or not it was within its bound. */
  ...
  for (const width of WIDTHS) {
    ...
    for (const [i, name] of names.entries()) {
      ...
      for (let col = 1; col < pngs.length; col++) {
        ...
        if (ratio > THRESHOLD) diverged.add(name);
  ...
  return { failures, known, diverged, compared };
}
...
try {
  const { failures, known, diverged, compared } = await run(browser, server.url);
  ...
  // would mask a future regression, so say so. Note this asks whether the component
  // diverged AT ALL — a component that blew past its bound is a failure, reported
  // below, not a stale entry.
  for (const name of Object.keys(INTENTIONAL)) {
    if (!diverged.has(name)) {
      console.error(
