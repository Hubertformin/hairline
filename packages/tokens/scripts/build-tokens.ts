...
const colors: Block = {
  ...
  lines: [
    ...
    '',
    '  /* the inverted surface — the one high-contrast tile per page */',
    declare('surface-inverse', palette['surface-inverse']),
    declare('text-on-inverse', palette['text-on-inverse']),
    '',
...
const typography: Block = {
  ...
  lines: [
    ...
    ...(['item', 'value', 'chip', 'tab', 'prefix', 'amount'] as const).map((k) =>
      declare(`type-${k}`, fontShorthand(text[k])),
    ),
...
/* The dark theme as a standalone stylesheet, so the framework-free layer can be
   dark too — a consumer linking hairline.css from a Django template shouldn't need
   Tailwind to get a dark mode. */
const darkCss = [
  '/* Generated by scripts/build-tokens.ts — do not edit. */',
  '',
  '/* Re-points the raw scale. The semantic aliases are unchanged, which is why this',
  '   is a short file: components read aliases, so nothing else has to move. */',
  ':root[data-theme="dark"], .dark{',
  ...Object.entries(darkPalette).map(([k, v]) => `  --${k}:${v};`),
  '}',
  '',
].join('\n');
writeFileSync(resolve(root, 'src/css/dark.generated.css'), darkCss);
console.log(`dark   → src/css/dark.generated.css (${Object.keys(darkPalette).length} overrides)`);
