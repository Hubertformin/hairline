## How to build with Ledger

**No provider, no wrapper, no theme setup.** Ledger is a plain CSS-custom-property
...
**1. Components** (`window.Ledger.*`) — always prefer these for UI parts:
`Button`, `IconButton`, `Chip`, `Badge`, `Field`, `Select`, `Toggle`,
`SegmentedTabs`, `Tile`, `StatBlock`, `DataRow`, `MeterBar`.

**2. Tokens + the `.led-` classes** for your own layout glue. There is no utility
...
`.led-board` (a white board on the desk), `.led-label` (the mono uppercase label),
`.led-data` (any figure in a table), `.led-marker` (the highlighter),
`.led-hairline` (a 1px rule).
...
- **Every figure in a table is mono and tabular** — `StatBlock`, `DataRow`, or
  `.led-data`. A figure never appears in the sans in a table; prose never appears
  in the mono.
