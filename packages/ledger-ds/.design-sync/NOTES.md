...
- `--entry ./dist/index.js` is required: `node_modules/ledger-ds` doesn't exist
...
- `cssEntry: dist/ledger.css` carries both tokens and component classes, so there is
...
- **The previews import from `'ledger-ds'` by package name.** If the package is ever
...
- **The preview content is Moni-specific** (XAF amounts, mobile money, school fees).
  That is deliberate — the design system was extracted from Moni — but if Ledger is
...
- **Not verified on a device**: the `ledger-ds/native` components are typechecked and
