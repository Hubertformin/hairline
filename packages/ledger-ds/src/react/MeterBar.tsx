...
export const MeterBar = forwardRef<HTMLDivElement, MeterBarProps>(function MeterBar(
  ...
  if (segments && segments.length) {
    ...
    return (
      <div
        ref={ref}
        className={cx('led-meter', 'led-meter--stacked', className)}
        style={sized}
        {...rest}
      >
        {segments.map((s, i) => (
  ...
  return (
    <div
      ...
      aria-valuemin={0}
      aria-valuemax={limit}
      className={cx('led-meter', over && 'led-meter--over', className)}
      style={sized}
      {...rest}
    >
      <span className="led-meter__fill" style={{ width: `${over ? 100 : pct}%` }} />
    </div>
  );
});
