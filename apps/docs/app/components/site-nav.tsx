...
export function SiteNav() {
  return (
    <header className="sticky top-0 z-20 border-b border-hairline bg-paper/85 backdrop-blur-sm">
      <div className="mx-auto max-w-6xl px-s9 h-14 flex items-center justify-between gap-s9">
        <a href="#top" className="flex items-center gap-s4 no-underline">
        ...
        <nav className="flex items-center gap-s3">
          {LINKS.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className="type-body-s text-muted hover:text-strong no-underline px-s5 py-s3 rounded-pill transition-[color,background-color] duration-fast ease-hairline hover:bg-surface-2"
            >
              {l.label}
            </a>
          ))}
