...
import { cn } from '@/registry/ledger/lib/utils';

/**
 * The one pressable shape in Ledger: a pill.
 *
 * `solid` is the single primary action per view — if two solid buttons are visible
 * at once, one of them is wrong.
 *
 * Every value here is a Ledger token: `h-control`, `rounded-pill`, `px-s8` and
...
export const buttonVariants = cva(
  'inline-flex items-center justify-center gap-s3 whitespace-nowrap border-0 cursor-pointer ' +
    'rounded-pill transition-[color,background-color] duration-fast ease-ledger ' +
    ...
    variants: {
      tone: {
        ...
        danger:
          ...
          'hover:bg-[color-mix(in_srgb,var(--lg-alarm-wash)_90%,var(--lg-alarm))]',
      },
      size: {
