...
export const StatBlock = forwardRef<HTMLDivElement, StatBlockProps>(function StatBlock(
  ...
  return (
    <div
      ...
      className={cx(
        'led-stat',
        size !== 'm' && `led-stat--${size}`,
        tone !== 'ink' && `led-stat--${tone}`,
        className,
      )}
      {...rest}
    >
      {label ? <div className="led-stat__label">{label}</div> : null}
      <div className="led-stat__row">
        <span className="led-stat__value">{value}</span>
        {unit ? <span className="led-stat__unit">{unit}</span> : null}
      </div>
      {note ? <div className="led-stat__note">{note}</div> : null}
    </div>
  );
});
