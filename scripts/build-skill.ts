...
cpSync(resolve(root, 'dist/ledger.css'), resolve(out, 'ledger.css'));
...
cpSync(resolve(root, 'dist/index.d.ts'), resolve(out, 'ledger.d.ts'));
 ...
 * that rots: an agent told about `.led-card` when the stylesheet says `.led-tile`
...
for (const raw of css.split('\n')) {
  ...
  if (line.endsWith('{')) {
    ...
    selectorBuffer = [];
    // Descendant rules (`.led-tile--dark .led-stat__value`) restyle a class in a
